...
# Helper funkcija za tablice koja radi sa svim formatima
make_table <- function(data, caption = NULL, col_names = NULL, align = NULL, full_width = FALSE) {
  if (is.null(col_names)) {
    tbl <- kable(data, caption = caption, align = align)
  } else {
    tbl <- kable(data, col.names = col_names, caption = caption, align = align)
  }
  
  # Samo za HTML primjenjuj bootstrap styling
  if (knitr::is_html_output()) {
    tbl <- tbl |> kable_styling(bootstrap_options = c("striped", "hover"), full_width = full_width)
  }
...
make_table(adj_matrix, align = c("l", rep("c", 4)))
```
...
make_table(degree_df, 
      col_names = c("Medij", "Tip", "Stupanj", "Normalizirani stupanj"),
      full_width = FALSE)
```
...
make_table(betweenness_df, 
      col_names = c("Medij", "Tip", "Međuposredovanje", "Normalizirano"),
      full_width = FALSE)
```
...
make_table(closeness_df, 
      col_names = c("Medij", "Tip", "Bliskost", "Normalizirana"),
      full_width = FALSE)
```
...
make_table(eigen_df, 
      col_names = c("Medij", "Tip", "Svojstvena vekt. centralnost"),
      full_width = FALSE)
```
...
make_table(all_centrality, 
      col_names = c("Medij", "Tip", "Stupanj", "Međuposr.", "Bliskost", "Eigenvector"),
      full_width = TRUE)
```
...
make_table(interpretation_df,
      col_names = c("Mjera", "Pitanje koje odgovara", "Primjer visokog rezultata", "Komunikacijski koncept"),
      full_width = TRUE)
```
...
make_table(density_examples,
      col_names = c("Tip mreže", "Tipična veličina", "Očekivana gustoća", "Interpretacija"),
      full_width = TRUE)
```
...
make_table(sw_comparison,
      col_names = c("Tip mreže", "Klasteriranje", "Prosječna putanja", "Primjer", "Implikacije za komunikaciju"),
      full_width = TRUE)
```
...
make_table(sf_implications,
      col_names = c("Aspekt", "Karakteristika", "Primjer u medijima"),
      full_width = TRUE)
```
...
make_table(macro_summary,
      col_names = c("Mjera", "Definicija", "Tipične vrijednosti", "Interpretacija"),
      full_width = TRUE)
```
...
make_table(clique_variants,
      col_names = c("Koncept", "Definicija", "Strogost", "Tipična primjena"),
      full_width = TRUE)
```
...
make_table(homophily_types,
      col_names = c("Dimenzija", "Manifestacija", "Posljedice", "Primjer"),
      full_width = TRUE)
```
...
make_table(community_algorithms,
      col_names = c("Algoritam", "Princip rada", "Prednosti", "Nedostaci"),
      full_width = TRUE)
```
...
make_table(constraint_examples,
      col_names = c("Karakteristika", "Pozicija u mreži", "Pristup informacijama", 
...
make_table(layout_algorithms,
      col_names = c("Algoritam", "Princip", "Optimalna veličina", "Prednosti"),
      full_width = TRUE)
```
...
Postoji niz strategija za suočavanje s efektom dlakave lopte. Prva strategija jest **filtriranje** mreže uklanjanjem manje važnih čvorova ili veza. Druga strategija jest **agregacija** čvorova u veće jedinice. Treća strategija jest korištenje **interaktivnih vizualizacija**. Četvrta strategija uključuje **vizualno kodiranje** dodatnih informacija kroz veličinu, boju i oblik čvorova.
...
make_table(hairball_solutions,
      col_names = c("Strategija", "Opis", "Prednosti", "Ograničenja"),
      full_width = TRUE)
```
...
**Gephi** predstavlja vjerojatno najpopularniji samostalni softver za vizualizaciju mreža. **UCINET** jest komercijalni softver s dugom tradicijom u analizi društvenih mreža. **R** s paketima poput **igraph**, **ggraph**, **tidygraph** i **statnet** nudi programatičan pristup mrežnoj analizi. **Python** s bibliotekama **NetworkX**, **graph-tool** i **pyvis** nudi sličnu programatičnu fleksibilnost.
...
make_table(tools_comparison,
      col_names = c("Alat", "Tip", "Cijena", "Prednosti", "Preporučeno za"),
      full_width = TRUE)
```
...
make_table(boundary_strategies,
      col_names = c("Strategija", "Opis", "Primjer", "Rizici"),
      full_width = TRUE)
```
...
make_table(missing_sources,
      col_names = c("Izvor", "Opis", "Tipični učinak", "Moguće rješenje"),
      full_width = TRUE)
```
...
make_table(privacy_techniques,
      col_names = c("Tehnika", "Opis", "Učinkovitost zaštite", "Utjecaj na korisnost"),
      full_width = TRUE)
```
...
make_table(methodology_summary,
      col_names = c("Izazov", "Ključno pitanje", "Potencijalne posljedice", "Ključne preporuke"),
      full_width = TRUE)
```
...
Analiza društvenih mreža predstavlja snažan metodološki pristup razumijevanju komunikacijskih fenomena kroz relacijsku perspektivu. Ovaj dokument pružio je sveobuhvatan uvid u temeljne koncepte, metode i izazove mrežne analize u kontekstu istraživanja masovne komunikacije.

Kroz poglavlja smo istražili relacijsku perspektivu koja naglašava važnost odnosa nad atributima, osnovne elemente mreža uključujući čvorove i veze te njihove različite tipove, mjere centralnosti koje identificiraju ključne aktere, strukturu mreže na makro razini s fenomenima poput malog svijeta i mreža bez skale, grupne strukture kao što su klike i zajednice, principe vizualizacije mreža, te metodološke probleme koji zahtijevaju pažnju istraživača.

Ključni uvidi uključuju razumijevanje da su društvene strukture više od zbroja individualnih atributa, da položaj u mreži oblikuje mogućnosti i ograničenja aktera, te da globalna svojstva mreža poput gustoće, klasteriranja i distribucije veza imaju važne implikacije za procese poput širenja informacija i formiranja javnog mnijenja.

Za buduća istraživanja, važno je balansirati između metodološke rigoroznosti i praktičnih ograničenja, biti svjestan etičkih izazova koje donosi rad s mrežnim podacima, te kontinuirano pratiti razvoj novih metoda i alata za mrežnu analizu. Analiza društvenih mreža nastavlja se razvijati kao disciplina, nudeći nove uvide u sve složenije komunikacijske ekosustave digitalnog doba.
